...
# TODO: Make it enumerate: "ORC" "IS" "SC"
for FAULT_MODEL in "SC"; do
    ...
    python3 ./fill.py "./FISSC/c1111 RISC-V (32-bits) gcc 14.2.0/VerifyPIN_0.asm" --memory $MEMORY --flips $FLIPS --template ./experiments/1111_template.xml --output $MODEL_PATH --fault_models $FAULT_MODEL --cooldown $COOLDOWN
    ...
    for PROGRAM in \
        ...
        MODEL_PATH="$BASE_DIRECTORY/$PROGRAM--F$FLIPS--C$COOLDOWN--T$TIME--N$SIMULATIONS--$FAULT_MODEL.xml"
        python3 ./fill.py "./FISSC/c1111 RISC-V (32-bits) gcc 14.2.0/$PROGRAM.asm" --memory $MEMORY --flips $FLIPS --template ./experiments/1111_template.xml --output $MODEL_PATH --fault_models $FAULT_MODEL --cooldown $COOLDOWN
